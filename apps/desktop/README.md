...
`tauri:dmg` runs `tauri:build` and then creates `target/release/bundle/dmg/FluxDown_0.1.0_aarch64.dmg` through the repository `scripts/create-macos-dmg.mjs` helper. The helper intentionally avoids Finder AppleScript so the DMG can be produced from CI and headless environments.
