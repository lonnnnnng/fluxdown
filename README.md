...
该流水线只允许在 GitHub Actions 页面手动触发。普通代码提交和 `v*` 标签推送都只同步代码或标签，不会自动启动打包；仅打包时保持 `publish_release=false`，发版时选择对应标签 ref 并设置 `publish_release=true`。Actions 页面中事件为 `push` 的历史记录来自旧版触发配置，当前配置不会因普通 push 继续新增这类运行。
