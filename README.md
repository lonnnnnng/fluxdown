...
补强 Android Release 协议验收取证、Torrent/Magnet 独立目录与 SHA-256 校验，并同步 macOS 当前 GUI 回归报告和跨平台验证文档。详见 [发行说明](docs/releases/1.0.26.md)。
...
### 1.0.24 更新（2026-09-26）

修复 iOS 发布流水线对 Git 依赖 `libtorrent_flutter 2.0.1` 的 Pub 缓存目录识别错误：构建脚本现在按锁定的 `resolved-ref` 查找 Git checkout，并兼容 hosted 包与 Flutter 插件链接。详见 [发行说明](docs/releases/1.0.24.md)。

### 1.0.22 更新（2026-09-12）

修复 iOS Release 流水线只补齐 simulator slice、未补齐 `ios-arm64` 真机 slice 的问题；现在会同时恢复并校验 device 与 simulator 两套静态库，确保 simulator 和 unsigned device 构建都能完成。详见 [发行说明](docs/releases/1.0.22.md)。
