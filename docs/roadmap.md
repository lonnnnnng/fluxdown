...
| 下载线程 | 桌面、移动和 CLI 设置支持 1-32，默认 8；HTTP Range 和 HLS 分片已接入并行下载。 | 不是所有协议都按线程数建立同样数量的连接。 |
| 失败重试 | 桌面与移动设置支持自动重试 0-10 次，默认 1；0 表示不重试。CLI `start/run` 有重试参数。 | 重试不包含首次尝试；不能消除资源失效、认证失败或无 peer 等外部原因。 |
| 网速限制 | 各端下载路径已有可选限速；桌面/core 支持每任务限速覆盖默认配置。 | 不配置时不主动限速，但不保证任何资源都能跑满带宽；字段名保留历史 `mbps`，界面和参数单位统一为 MiB/s。 |
