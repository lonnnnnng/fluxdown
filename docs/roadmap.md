...
| 编号 | 方向 | 交付条件 |
| --- | --- | --- |
| P2-01 | 移动下载引擎逐步收敛到 Rust | 已补非阻塞单任务/队列运行句柄、状态轮询、暂停/继续/重置/删除、句柄回收，以及队列并发、线程数、重试次数和限速参数透传；`RustQueueBackend` 已可注入 `DownloadController`，按任务 ID、进度、速度、错误、真实文件名和时间戳回写 Flutter，native 初始化/入队失败可回退 Dart。默认生产入口仍由 Dart/原生适配器执行，下一步是整队列 schema 迁移、默认切换和 Android/iOS 真机回归。 |
