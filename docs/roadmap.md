...
核对日期：2026-09-25。源码基线：`main` / 当前工作树（P1-05 错误处理与异常恢复）；当前发行版：[`1.0.23`](releases/1.0.23.md)。本文按源码汇总功能，运行结论引用已有报告和本轮验证，不把隔离 UI 或命令层结果写成原生 GUI/移动端全量验收。
