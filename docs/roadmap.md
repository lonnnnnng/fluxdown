...
核对日期：2026-09-08。源码基线：`main` / `213ddb1`；当前发行版：[`1.0.17`](releases/1.0.17.md)，发布代码为 `d33d716`。本文按源码汇总功能，运行结论引用已有报告，本次文档更新没有重新执行 App 或全协议测试。
...
| 并发排队 | 桌面与移动设置支持 1-30 个并发，默认 1；超出并发数的任务排队。CLI `run` 可指定并发。 | 手动暂停、已移交和失败任务需按各自状态处理，不等同于等待下载。 |
| 下载线程 | 桌面与移动设置支持 1-32，默认 8；HTTP Range 和 HLS 分片已接入并行下载。 | 不是所有协议都按线程数建立同样数量的连接；CLI `--threads` 当前默认 1。 |
| 失败重试 | 桌面与移动设置支持自动重试 0-10 次，默认 1；0 表示不重试。CLI `start/run` 有重试参数。 | 重试不包含首次尝试；不能消除资源失效、认证失败或无 peer 等外部原因。 |
| 网速限制 | 各端下载路径已有可选限速；桌面/core 支持每任务限速覆盖默认配置。 | 不配置时不主动限速，但不保证任何资源都能跑满带宽；单位文案和跨后端作用域仍需统一。 |
...
| 保存位置 | 设置默认目录，单任务可覆盖；当前为路径文本输入。 | 系统目录选择、保存与回显、单任务覆盖、磁盘总量/已用/可用展示。 | 输出路径参数与队列存储路径参数。 |
| 数量与速度设置 | 数字输入配置并发、线程、重试、限速并持久化。 | 数字输入配置并发、线程、重试、限速并持久化。 | 由命令参数配置；默认值和参数范围不应直接等同 GUI。 |
| Torrent 多文件选择 | 可输入文件编号下载选定文件，尚无新建时的文件树勾选。 | metadata 解析后弹出多文件选择，保存选择结果，支持文件夹详情与已落盘文件预览。 | 重复传入 `--torrent-file-index` 选择文件。 |
| Torrent 详情 | 文件清单、逐文件已下载量/百分比、tracker、peer、会话速度/ETA；静态 metadata 进度显示未知。 | 文件清单与任务总进度；下载中逐文件完成字节未知，尚无真实逐文件速度。 | 当前以任务级输出为主，无对应详情命令。 |
| HLS | 清晰度 variant 选择、分片缓存恢复、可选保留 TS；默认尝试 FFmpeg 转 MP4，失败保留 TS。 | VOD、AES-128、fMP4、BYTERANGE、TS 转 MP4；当前 master 默认首个 variant。 | 复用 core 默认下载路径，尚无 variant/保留 TS 的命令行选项。 |
...
- **发行策略**：只有明确打包/发版后才手动运行流水线；公开 11 个上传文件，加 GitHub 自动源码包共 13 项。内部调试、商店和 iOS 验证产物保留 Actions Artifacts；已有资产白名单、缺包、大小及 SHA-256 回验。
...
| P1-02 | Torrent 文件夹体验补齐 | 桌面选择需输编号，文件行无预览；移动端下载中没有真实逐文件进度/速度。 | 桌面解析后可勾选文件树；各端选择可持久化、重启可恢复；按后端实际数据展示文件进度/速度，未知时明确显示未知，已落盘文件可打开。 |
| P1-03 | HLS 配置跨端对齐 | 桌面有 variant/保留 TS，移动和 CLI 缺少入口；桌面转封装回退信息不够明确。 | 移动新建和 CLI 可选择 variant/输出策略；MP4/TS 名称与真实格式一致；缺 FFmpeg、解密和转封装失败有明确结果，覆盖恢复及对应格式测试。 |
| P1-04 | 设置与保存位置一致性 | 桌面目录为文本输入；CLI 线程默认 1，与 GUI 的 8 不同；限速存在 Mbps/MB/s 历史命名差异。 | 桌面加入原生目录选择；明确并统一默认值、范围、字节单位及全局/任务/后端限速作用域，兼容旧配置；验证设置持久化、权限失效和并发任务限速。 |
| P1-05 | 错误处理与异常恢复 | 已有暂停/重试/原子保存测试，但公网与设备异常仍需扩展。 | 对断网、认证失败、无 peer、磁盘满、目录权限撤销、进程中断给出可操作提示；补 HLS/FTP/FTPS/SFTP/SMB 及队列的针对性回归，不能把错误显示为完成或假进度。 |
