...
- 并发、线程、重试和限速口径：并发 `1-30`（默认 `1`）、单任务线程 `1-32`（默认 `8`）、自动重试 `0-10`（默认 `1`，不含首次尝试）、限速留空表示不限速，单位为 `MiB/s`。CLI `download/start/run` 的 `--threads` 默认已统一为 `8`；旧字段 `speed_limit_mbps`/`speedLimitMbps` 保留兼容，实际按 `MiB × 1024²` 换算。
