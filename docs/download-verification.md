...
| `cargo test -p fluxdown-core -p fluxdown-cli -p fluxdown-desktop` | 通过：CLI 单元 1、CLI 集成 26、core 63、desktop 22，desktop 另有 5 个需 live fixture 的 ignored 用例；新增覆盖直连下载 SHA-256 成功/失败、非法 hash 在 `--restart` 清理前失败、队列任务 SHA-256 持久化和失败状态，以及桌面 command 层 SHA-256 成功和 mismatch 失败。 |
