...
- Rust core 新增共享默认常量，CLI、Tauri 后端和移动 FFI 共同引用；Flutter 设置页、控制器和下载器改为引用同一个移动端默认配置，桌面设置的读取和输入回退统一引用 `defaultSettings`。
- 已保存的用户配置继续优先，不会因为升级被覆盖；只有未保存或缺失的配置项使用新默认值。队列仍按并发槽位调度，重试次数不含首次尝试，且仅瞬态错误消耗重试次数。
- 自动化验证：Rust core `84` 项、CLI 单元 `2` 项和集成 `36` 项、Tauri 非忽略 `35` 项全部通过（另有 `7` 项外部 fixture 用例保持忽略）；Flutter `57` 项通过，`4` 项需真实 FFI 库的用例按既有条件跳过；`flutter analyze` 和桌面 TypeScript/Vite 构建通过。当前 ESLint 仍被 4 个既有 React purity 错误阻断，与本轮默认值修改行无关。
...
- 当时（2026-09-09）的并发、线程、重试和限速口径为：并发 `1-30`（默认 `1`）、单任务线程 `1-32`（默认 `8`）、自动重试 `0-10`（默认 `1`，不含首次尝试）、限速留空表示不限速，单位为 `MiB/s`。该历史快照已由 2026-09-10 记录的新默认值取代；旧字段 `speed_limit_mbps`/`speedLimitMbps` 保留兼容，实际按 `MiB × 1024²` 换算。
- 已完成代码级回归：桌面后端默认线程与边界测试、CLI 默认线程和限速边界测试、移动设置解析测试、Cargo/Flutter/桌面前端构建检查。当前仍需在目标桌面环境手工点击目录对话框，并补一条目录权限撤销后的真实错误证据；本节不把静态代码检查写成完整 GUI 验收。
...
- 新增剪贴板监听：检测到复制的可下载链接时顶部提示条一键创建任务（Rust 端读取系统剪贴板，不受 WebView 权限限制，可在设置关闭）。
- Windows GUI E2E 9 项用例全部通过：托盘构建后应用存活、第二实例自动退出且首实例存活、剪贴板提示条显示/接受预填/忽略消失、通知命令真实调用成功、设置页新增两个开关。
- Rust 测试 140 项全部通过（core 75 / desktop 33 / cli 31）。
...
2026-06-23 04:56 CST 为 `scripts/verify-ios-integration.sh` 增加 iOS TS HLS 专项探针：默认 smoke 不启用该用例，保持 HTTP/fMP4 HLS/fMP4 BYTERANGE HLS 通过；显式设置 `FLUXDOWN_IOS_INCLUDE_TS_HLS=1` 时会生成视频+AAC 的 MPEG-TS HLS 并要求输出 MP4。当时 simulator 结果仍失败，`ios-hls-ts-local` 下载了 `15604` bytes 后在原生 remux 阶段返回 `AVFoundationErrorDomain -11838`，错误原因为 `The operation is not supported for this media.`。
...
2026-06-30 01:49 CST 在 Windows 开发机新增并运行 `python scripts/verify-windows-cli-protocols.py --keep-work-dir` 通过：覆盖 HTTP、HTTPS、WebDAV、WebDAVS、FTP、FTPS、m3u8/HLS、SFTP、SMB、Torrent、Magnet 和 ed2k 共 12 种当前支持协议。除 ed2k 为系统移交验证外，其余协议均完成真实文件落盘和 SHA-256 校验；原始 JSON 证据见 `docs/artifacts/windows-cli-protocol-e2e-20260630.json`，汇总报告见 [Windows CLI 12 协议真实下载验证报告](protocol-e2e-windows-report-20260630.md)。

2026-06-30 08:54 CST 在 Windows 开发机新增并运行 `python scripts/verify-windows-desktop-gui-protocols.py --app target/e2e-gui/debug/fluxdown-desktop.exe --keep-work-dir` 通过：覆盖原生 Tauri 桌面窗口前台操作下的 HTTP、HTTPS、WebDAV、WebDAVS、FTP、FTPS、m3u8/HLS、SFTP、SMB、Torrent、Magnet 和 ed2k 共 12 种当前支持协议。除 ed2k 为系统移交验证外，其余协议均通过 GUI 创建任务、点击开始队列、真实文件落盘和 SHA-256 校验；设置页验证覆盖基础设置、下载策略、协议能力、存储与完成、安全与隐私、高级诊断六个菜单。原始 JSON 证据见 `docs/artifacts/windows-desktop-gui-protocol-e2e-20260630.json`，截图见 `docs/artifacts/windows-desktop-gui-queue-20260630.png` 和 `docs/artifacts/windows-desktop-gui-settings-20260630.png`，汇总报告见 [Windows 原生 Tauri GUI 12 协议真实下载验证报告](protocol-e2e-windows-desktop-gui-report-20260630.md)。

2026-07-02 23:01 CST 为发布 `1.0.5` 完成发版前本地复验：版本号已同步到 `package.json`、`apps/desktop/package.json`、Rust workspace、Tauri 配置和 Flutter `1.0.5+6`；`cargo test --workspace`、`npm --workspace apps/desktop run build`、`npm run mobile:analyze`、`npm run mobile:test`、严格 `cargo clippy --workspace --all-targets -- -D warnings`、`npm run verify:ci-config`、`npm run verify:macos`、`npm run verify:ios`、`npm run mobile:android:release`、`npm run mobile:android:aab` 和 `npm run verify:artifacts` 均已通过。本轮生成并校验 `target/release/bundle/dmg/FluxDown_1.0.5_aarch64.dmg`、`apps/mobile/build/app/outputs/flutter-apk/app-release.apk`、`apps/mobile/build/app/outputs/bundle/release/app-release.aab`、iOS debug frameworks、iOS simulator app 和 unsigned iPhone device app。`npm run audit:release` 的代码覆盖和本地 macOS/Android/iOS 产物检查均通过，失败项仅为本机未生成的 Linux/Windows 跨平台产物和 staged release manifest，这些由手动触发的 GitHub Actions release workflow 在对应 runner 上生成。
...
2026-08-04 完成当前蓝白主题 UI 复验：macOS 启动真实 Tauri 开发窗口，在逻辑尺寸 `1280×820` 下采集下载列表、新建任务和设置页三张 Retina 截图（PNG `2560×1640`）；Android 构建并安装 `1.0.8+9` release APK 到 Redmi Note 8 Pro 真机，前台打开任务页、新建弹框和设置页并采集三张 `1080×2340` 截图。两端均确认天蓝色/白色主题、常规字重、菜单可读性和主要页面无溢出。本轮仅验证构建、安装、启动、页面操作和视觉结果，没有重跑协议下载，也没有把历史 `1.0.4` Android 真机协议结果改记为当前版本结果。
...
FluxDown 已经具备多端架构、构建产物、CI/Release artifact 校验、核心协议下载测试、Android 真机 App 下载验证、macOS CLI/GUI 本地协议真实下载验证，以及 Windows CLI/原生 Tauri GUI 12 协议真实验证，但尚未完成每个平台、每个协议、每种 GUI/App 前台路径的真实下载端到端验证。
...
| `flutter test` | 通过：36 个移动端测试全部通过，覆盖协议识别、队列状态、并发、重试、限速、线程数、HTTP/WebDAV/TS HLS/fMP4 HLS/fMP4 BYTERANGE HLS/FTP 下载和续传；本机有 ffmpeg 时会生成真实 MPEG-TS HLS，并用 ffprobe 校验输出 MP4。 |
...
| `cargo test -p fluxdown-core protocol::tests:: -- --nocapture` + `cargo test -p fluxdown-cli --test download_command -- --nocapture` | 通过：CLI `detect` 输出改为与队列 JSON 一致的稳定小写协议名，覆盖 HTTP/HTTPS/WebDAV/FTP/SFTP/SMB/Torrent/Magnet/ed2k/m3u8/unknown。 |
...
| `target/release/fluxdown doctor` | 通过；内建 HTTP/HTTPS/WebDAV/FTP/FTPS/Torrent/Magnet/m3u8/SFTP/SMB 可执行；ed2k 为系统移交，当前 PATH 缺少可选 `ed2k` CLI。 |
...
| Tauri command 单任务启动 | 通过，测试启动临时 HTTP、HLS、WebDAV transport gateway fixture，隔离 `XDG_DATA_HOME` 队列路径，创建任务后调用 `start_download`；HTTP/WebDAV 校验任务直接进入 `finished`、`task_output_path` 指向真实文件且内容正确，HLS 校验最终产物名回写、`segments_written=2` 且内容与分片拼接结果一致；`restart_existing=true` 校验已完成 HTTP 任务会重新请求源文件并替换旧输出。 |
