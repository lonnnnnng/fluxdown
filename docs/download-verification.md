...
验证结束后已确认无 `http.server 8765`、`local_bt_tracker`、`transmission-daemon`、`fluxdown-desktop` 残留进程，`8765`、`6969`、`51413` 端口无监听；CLI HTTP/HLS 脚本使用临时随机端口并在退出时清理 HTTP server 和临时目录，CLI 和桌面 P2P 脚本也会清理 tracker、Transmission 和临时目录。
