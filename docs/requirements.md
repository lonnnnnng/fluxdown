...
- 输入下载源、输出目录和可选文件名。
- 可选输入 SHA-256；配置后下载完成必须校验最终文件，校验失败进入 `failed`。
- 展示协议识别和支持状态。
- 添加任务到本地队列。
...
- ed2k 链接移交给系统中已安装的兼容 App；移动端移交成功后记录 `handedOff` 状态。
...
3. 系统选择内建后端或外部移交后端。
4. 下载器写入目标文件。
5. 如果用户提供 SHA-256，系统校验最终文件；校验通过后摘要返回实际 SHA-256，校验失败则返回错误。
6. 完成后返回协议、后端、输出路径、写入字节数、断点续传起点和总大小。
...
5. 如果任务配置了 SHA-256，系统在下载完成后校验最终文件。
6. 内建下载完成后状态变为 `finished`；失败后状态变为 `failed` 并记录错误；暂停后状态变为 `paused`。移动端 ed2k 成功交给外部兼容 App 后状态为 `handedOff`，仅表示移交成功，不表示 FluxDown 已完成文件下载。
