...
CI 实测：macOS CLI 从 `15,732,112` 字节降至 `9,305,648` 字节，减少 40.8%；Windows CLI 从 `15,017,984` 降至 `12,232,704` 字节，减少 18.5%。优化后的 host Release FFI 已通过 50 项 Flutter 测试，含真实协议识别、队列和本地 HTTP 文件大小/内容核验。

公开/内部产物同架构基线对比（字节，压缩前后的同类文件）：

| 产物 | `1.0.15` 基线 | `1.0.16` | 变化 |
| --- | ---: | ---: | ---: |
| Android APK | 149,578,719 | 140,762,569 | -5.9% |
| Android AAB（Actions Artifact） | 133,279,913 | 124,838,542 | -6.3% |
| macOS DMG | 8,954,324 | 6,416,681 | -28.3% |
| Linux DEB | 10,188,670 | 7,629,570 | -25.1% |
...
本轮没有启动 GUI 或设备、没有改动用户队列；原生 App 全协议真运行仍需单独验收。发布流水线和远端资产回验记录见 [下载验证状态](download-verification.md)。
