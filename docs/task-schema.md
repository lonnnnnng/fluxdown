...
| `created_at_ms` 等 | `createdAt` 等 `DateTime` | Rust FFI 投影会将 `started_at_ms`/`finished_at_ms` 转为 UTC `DateTime`；移动端持久化仍使用 ISO-8601，尚未提供整队列导入/导出迁移 |
...
`total_bytes`、`downloaded_bytes`、运行速度、错误和 Rust 毫秒时间戳。它仍不是完整 Rust 任务，也不是移动端 `DownloadTask` 的替代物。
