...
当前 FFI 队列接口会读取并映射 Torrent metadata、`hlsVariantIndex` 与 `hlsKeepTransportStream`；移动端下载器同时通过自己的 camelCase 队列字段执行这些选项。FFI 已提供同步和非阻塞运行投影，但不能把 Rust 运行器的全局并发、线程数、重试或限速配置视为已由移动端透传。
