...
流水线只在 GitHub Actions 页面手动触发 `workflow_dispatch` 时运行。普通代码提交推送到 `main` 不触发打包流水线；推送 `v*` 标签也只同步标签，不自动触发流水线。需要打包时手动运行 workflow 并保持 `publish_release=false`；需要发版时手动选择对应 `v*` 标签 ref，并设置 `publish_release=true`。
