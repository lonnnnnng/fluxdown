...
- `release`：标签 `v*` 触发，下载所有 artifact，整理 assets，发布或更新 GitHub Release。

流水线只在两类场景运行：推送 `v*` 标签，或在 GitHub Actions 页面手动触发 `workflow_dispatch`。普通代码提交推送到 `main` 不触发打包流水线；需要发版或打包时再创建标签或手动运行。
...
6. GitHub Actions 自动构建多端产物并发布 Release。
