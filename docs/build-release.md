...
当前版本号为 `1.0.24`，修复 iOS 发布流水线识别 `libtorrent_flutter` Git checkout 的问题，Android Torrent/Magnet 完成状态轮询和真实多文件真机验证仍包含在当前基线中，见 [发行说明](releases/1.0.24.md)。发布标签使用 `v<version>`，GitHub Release 作业会校验标签版本和 `package.json` 版本一致。`v1.0.15` 因 Linux CLI 回归失败未发布，保留原标签；后续发版使用新版本号，不覆盖已有标签。
