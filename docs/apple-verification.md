...
2026-07-02 23:01 CST 为发布 `1.0.5` 复跑 Apple 侧和移动端发版前验证：`npm run verify:macos` 通过，覆盖 `cargo fmt --check`、严格 Clippy、core/CLI/desktop 测试、release CLI HTTP/HLS/FTP/FTPS/SFTP/SMB/Torrent/Magnet/队列控制真实 fixture、macOS 桌面 command fixture、`FluxDown.app`、`FluxDown_1.0.5_aarch64.dmg`、许可证和 CI 手动触发策略检查；`npm run verify:ios` 通过，覆盖 Flutter analyze/test、iOS debug frameworks、simulator app、unsigned device app 和移动端 URL scheme；`npm run mobile:android:release` 与 `npm run mobile:android:aab` 通过，生成 Android release APK/AAB。`npm run verify:artifacts` 通过，确认 macOS CLI/桌面、Android APK/AAB、iOS framework/simulator/unsigned device 产物均存在且非空。iPhone 真机和签名 IPA 仍受签名材料与设备窗口限制，不作为本轮 `1.0.5` 发版阻塞项。
