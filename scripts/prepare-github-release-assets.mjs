...
- 桌面端与移动端统一为天蓝色、白色主题和常规字重，降低页面视觉重量并保持跨端风格一致。
- 桌面侧栏、状态菜单、协议能力卡和设置导航提升文字与背景对比度，选中态和计数信息更清晰。
- macOS 真实 Tauri 窗口与 Redmi Note 8 Pro 真机 release APK 已完成下载列表、新建任务和设置页视觉复验。
- README 已替换 macOS 与 Android 最新截图，中文、英文说明和验证边界同步更新。
