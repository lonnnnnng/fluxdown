...
printf 'fluxdown cli sftp sample\n' > "$UPLOAD_DIR/$SAMPLE_NAME"
EXPECTED_SIZE="$(wc -c < "$UPLOAD_DIR/$SAMPLE_NAME" | tr -d ' ')"
EXPECTED_SHA256="$(shasum -a 256 "$UPLOAD_DIR/$SAMPLE_NAME" | awk '{print $1}')"
SOURCE="sftp://flux:fluxpass@127.0.0.1:$SFTP_PORT/upload/$SAMPLE_NAME"
...
DIRECT_JSON="$TMP_DIR/sftp-direct.json"
...
assert_sha256 "$DIRECT_DIR/direct-sftp.txt" "$EXPECTED_SHA256"

ADD_JSON="$TMP_DIR/sftp-add.json"
RUN_JSON="$TMP_DIR/sftp-run.json"
LIST_JSON="$TMP_DIR/sftp-list.json"
